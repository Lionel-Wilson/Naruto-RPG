...
#Game setup
wind_width = 900 #wind = window
wind_height = 640#maybe try 540
BG_colour =(255,215,0)
# this variable is what opens up an empty window to run the game on.Note - that 0,0 cordinate starts from top left. bottom right is wind_width,wind_height. This is the surface that we draw things on.
window = pygame.display.set_mode((wind_width,wind_height)) 
Program_name = pygame.display.set_caption("Naruto RPG by East meets West") # This gives the program a name. This will be displayed on the top of the window
...
class Player_Platform_Mode:
    ...
    def draw(self):
         ...
         if self.left: #if left is =True then the below code will be run.
             if self.jumping: #checks if character is jumping
                 ...
                 if self.jumpcount >= 2:
                     window.blit(self.jumping_left_animation[0],(self.x_pos,self.y_pos)) 
                 elif self.jumpcount <2 and  self.jumpcount>=-2:
                     window.blit(self.jumping_left_animation[1],(self.x_pos,self.y_pos))
                 elif self.jumpcount <-2 and self.jumpcount>=-4:
                     window.blit(self.jumping_left_animation[2],(self.x_pos,self.y_pos))
                 elif self.jumpcount <=-4 and  self.jumpcount>-8:
                     window.blit(self.jumping_left_animation[3],(self.x_pos,self.y_pos))
                 else: #otherwise draw final jump animation
                     window.blit(self.jumping_left_animation[4],(self.x_pos,self.y_pos)) 
                     
             #RUNNING LEFT ANIMATION        
             else: 
                 window.blit(self.left_running_animation[self.runcount//3],(self.x_pos,self.y_pos+6))
                 self.runcount += 1
             ...
             if self.jumping: #checks if character is jumping
                 #cycles through the different jump animations
                 if self.jumpcount >= 2:
                     window.blit(self.jumping_right_animation[0],(self.x_pos,self.y_pos)) 
                 elif self.jumpcount <2 and  self.jumpcount>=-2:
                     window.blit(self.jumping_right_animation[1],(self.x_pos,self.y_pos))
                 elif self.jumpcount <-2 and self.jumpcount>=-4:
                     window.blit(self.jumping_right_animation[2],(self.x_pos,self.y_pos))
                 elif self.jumpcount <=-4 and  self.jumpcount>-8:
                     window.blit(self.jumping_right_animation[3],(self.x_pos,self.y_pos))
                 else: #otherwise draw final jump animation
                     window.blit(self.jumping_right_animation[4],(self.x_pos,self.y_pos)) 
                     
             #RUNNING RIGHT ANIMATION  
             else: 
                 window.blit(self.right_running_animation[self.runcount//3],(self.x_pos,self.y_pos+6))
                 self.runcount += 1
             ...
             window.blit(self.stance,(self.x_pos,self.y_pos))
    ...
    def key_inputs(self):
        ...
        if keys[pygame.K_LEFT] and self.x_pos>self.speed:
            ...
            self.left = True
            self.right = False
        elif keys[pygame.K_RIGHT] and self.x_pos<(wind_width-self.width-self.speed): #this ensures the character doesnt go off screen
        ...
        if not (self.jumping):#If you're not jumping then you're able to crawl(!add feature later!) and jump
            if keys[pygame.K_DOWN] and self.y_pos>(wind_width-self.height-self.speed):
...
class Player_Battle_Mode(Player_Platform_Mode):
    def __init__(self,norm_attack= 'Naruto Combo',spec_attack1 = 'Naruto Rendan',ep_restore_move = 'Charge Chakra'): 
        ...
        Player_Platform_Mode.__init__(self,name = 'Naruto Uzumaki',hp = 100,max_hp=100)
        self.x_pos=337
        self.y_pos=295
        ...
        if self.turn == True and target.turn == False:
            ...
            if not(self.attacking) and not (self.chakra_charging): 
                ...
                if keys[pygame.K_1]:
                    ...
                    print(f"\n{self.name} Lvl 1 \t\t{target.name} Lvl 10 \nHP: {self.hp}\t\t\t\tHP: {target.hp}\nEP: {self.ep}\n")
                if keys[pygame.K_2]:
    ...
    def normal_attack(self,target):
        ...
        if miss_chance<=80: #below is what happens when i dont miss
            if critical_chance<=10: #checks if i land critical. 1/10 chance
                if self.ep>=self.na_cost: #checks if I have enough energy to even use the normal attack and land the critical
                    ...
                    print(f"{self.name} attacked {target.name} with {self.norm_attack} and landed a CRITICAL HIT! Dealing a total of {total_dmg} damage!")
                    ...
                    self.attacking = False
                    self.na_attacking = False
                    print(f"{na_no_energy_msg}") #ADD NO ENRGY BOOLEAN IF POSSIBLE
                    return
            else: #what happens if its not a critical hit
                if self.ep>=self.na_cost:
                    self.ep = self.ep-self.na_cost
                    print(f"{self.name} attacked {target.name} with '{self.norm_attack}' and dealt {self.na_dmg} damage")
        ...
        #what happens if you miss        
            ...
            if self.ep>=self.na_cost:#check if you have enough energy
                self.ep = self.ep-self.na_cost
                print(f"{self.name} tried to use '{self.norm_attack}' on {target.name} but MISSED! {self.name}'s guard is now OPEN!")
                return 
            else:
                self.attacking = False
                self.na_attacking = False
    ...
    def special_attack1(self,target):
        ...
        if miss_chance<=80: #below is what happens when i dont miss
            if critical_chance<=10: #checks if i land critical. 1/10 chance
                if self.ep>=self.sa1_cost: #checks if I have enough energy to even use the normal attack and land the critical
                    ...
                    print(f"{self.name} attacked {target.name} with {self.spec_attack1} and landed a CRITICAL HIT! Dealing a total of {total_dmg} damage!")
                    target.hp = target.hp-total_dmg
                    if target.hp <=0: #checks if target is dead after that attack
                        target.alive = False
                        print(f"\n{self.name}\t\t\t{target.name}\nHP: {self.hp}\t\t\tHP: 0\nEP: {self.ep}\n")
                ...
                if self.ep>=self.sa1_cost:
                    self.ep = self.ep-self.sa1_cost
                    print(f"{self.name} attacked {target.name} with '{self.spec_attack1}' and dealt {self.sa1_dmg} damage")
        ...
        #what happens if you miss        
            ...
            if self.ep>=self.sa1_cost:#check if you have enough energy
                self.ep = self.ep-self.sa1_cost
                print(f"{self.name} tried to use '{self.spec_attack1}' on {target.name} but MISSED! {self.name}'s guard is now OPEN!")
                return 
            else:
                self.attacking = False
                self.sa1_attacking = False
    ...
    def energy_restoration(self):
        ...
        if self.ep+self.ep_restore<=self.max_ep:
            ...
            print(f"{self.name} restored {self.ep_restore} EP with '{self.ep_restore_move}'")
            return
        else:
            self.energy = self.max_ep
            print(f"{self.name} used '{self.ep_restore_move}' and maxed out their HP")
            return
    ...
    def draw_battle(self,enemy):
        if self.chakra_charging:
            if self.framecount + 1 >= ((len(self.chakra_charge_animation))*4):
                ...
                enemy.turn = True
                return
            window.blit(self.chakra_charge_animation[self.framecount//4],(self.x_pos,self.y_pos))
            self.framecount+=1
        ...
        #Checks if 'self'(Player) is attacking.
            ...
            if self.missing:
                if self.na_attacking:
                    if self.framecount + 1 >= ((len(self.na_animation))*3):
                        ...
                        enemy.turn = True
                        return
                    window.blit(self.na_animation[self.framecount//3],(enemy.x_pos-self.width,self.y_pos))
                    self.framecount+=1
                    return
                if self.sa1_attacking:
                    ...
                    if self.framecount + 1 >= ((len(self.sa1_animation))*3):
                        ...
                        enemy.turn = True
                        return
                    window.blit(self.sa1_animation[self.framecount//3],(enemy.x_pos-self.width,self.y_pos))
                    self.framecount+=1
                    return
            ...
            #This section checks what attack 'self' is attacking with(Attack checker and drawer can be put here maybe.
                ...
                if self.framecount + 1 >= ((len(self.sa1_animation))*3):
                    ...
                    enemy.turn = True
                    return
                window.blit(self.sa1_animation[self.framecount//3],((enemy.x_pos-enemy.width),enemy.y_pos))
                self.framecount+=1
                return
        else:
            window.blit(self.stance,(self.x_pos,self.y_pos))
    ...
    def draw_health_bar(self,enemy):
        window.blit(self.name_tag,(self.x_pos,(self.y_pos+150))) 
        window.blit(enemy.name_tag,(enemy.x_pos,enemy.y_pos+150))  
        if self.hp>=40 and self.hp<80:
            window.blit(self.hp_bars[0],(self.x_pos+self.hp_bar_x,self.y_pos+self.hp_bar_y+150))
        if self.hp<40:
            window.blit(self.hp_bars[1],(self.x_pos+self.hp_bar_x,self.y_pos+self.hp_bar_y+150))
        
        if enemy.hp>=40 and enemy.hp<80:
            window.blit(enemy.hp_bars[0],(enemy.x_pos+enemy.hp_bar_x,enemy.y_pos+enemy.hp_bar_y+150))
        if enemy.hp<40:
            window.blit(enemy.hp_bars[1],(enemy.x_pos+enemy.hp_bar_x,enemy.y_pos+enemy.hp_bar_y+150))
...
class Platform_NPC(): #Any characters that are not the main characters will be made from this class when in platform mode.
    def __init__(self,name = 'Kakashi Hatake',x_pos = 300,y_pos = 300,width = 26,height = 38,speed = 5 ):
       self.name=  name
       self.x_pos = x_pos 
       self.y_pos = y_pos
       self.width = width
...
class Battle_NPC(Platform_NPC):
    def __init__(self,norm_attack= 'Kakashi Combo A',spec_attack1 = 'Kakashi Combo B',na_dmg=8, sa1_dmg=12,critical_dmg=15):
        #ENEMY BATTLE STATS
        Platform_NPC.__init__(self,name ='Kakashi Hatake',x_pos = 563,y_pos = 290)
            ...
            if self.missing:
                if self.na_attacking:
                    if self.framecount + 1 >= ((len(self.na_animation))*3):
                        ...
                        enemy.turn = True
                        return
                    window.blit(self.na_animation[self.framecount//3],(enemy.x_pos+enemy.width,self.y_pos))
                    self.framecount+=1
                    return
                if self.sa1_attacking:
                    if self.framecount + 1 >= ((len(self.sa1_animation))*3):
                        ...
                        enemy.turn = True
                        return
                    window.blit(self.sa1_animation[self.framecount//3],(enemy.x_pos+enemy.width,self.y_pos))
                    self.framecount+=1
                    return
        ...
        if self.turn == True:
            ...
            if miss_chance<=80: #below is what happens when i dont miss
                if critical_chance<=12: #checks if i land critical. 1/10 chance
                    ...
                    print(f"{self.name} attacked {target.name} with '{self.spec_attack1}' and dealt {self.na_dmg} damage")
                    target.hp = target.hp-self.na_dmg
                    if target.hp <=0:
                        target.alive = False
                        print(f"Well done! You defeated {target.name}")
...
class Game_state: 
    ...
    def State_updater(self,Platform_player,Battle_player,Platform_NPC,fight_npc,map_name,previous_state, next_state = 'stage_1_tutorial', battle = None):
        if self.state == 'Title':
            ...
            window.blit(map_name.img,(0,0))
            window.blit(Platform_player.portrait,(50,20))
    ...
    def state_manager(self): #Change 'game state' to self as you're refering to the object and not the general class!
        ...
        if self.state == 'Title':
            self.State_updater(Naruto_pf,Naruto_battle,Kakashi_pf,Kakashi_battle,Title,None,'stage_1_tutorial')
             ...
             self.State_updater(Naruto_pf,Naruto_battle,Kakashi_pf,Kakashi_battle,Stage_2_minigame,'stage_1_tutorial','stage_5_sasuke_platform_p1')
